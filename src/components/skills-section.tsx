...
function SkillCategory({ title, skills }: { title: string; skills: Skill[] }) {
  return (
    <div className="bg-card rounded-lg p-4 border border-border shadow-sm hover:shadow-glow transition-shadow duration-300">
...
function SkillBadge({ name }: { name: string }) {
  return (
    <span className="inline-flex px-3 py-1 rounded-full text-sm bg-secondary text-primary hover:bg-primary/20 transition-colors duration-300 cursor-default">
      {name}
    </span>
  )
}
